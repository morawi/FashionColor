...
# from get_data_torch import ImageFolder
...
def remove_image_background(image, mask, filter_image=False, verbose=True):
    ...
    peaked_colors = colors_via_3d_hist(image_no_bkg, verbose=verbose)
...
def get_images_from_wardrobe(path_to_wardrobe= '../Data/Wardrobe/', 
                             user_name='malrawi',
                             resize_enabler=True, 
                             verbose=False):
    ...
    adding more filters to the filter banks. Currently, this implementation only supports
    ...
    for fname in fnames:
        ...
        record['all_images_no_bkg'] = remove_image_background(img, mask = mask, verbose=verbose)
...
def resize_image(img, mask, scale_factor = 0.5):           
    ...
    interpol_method = cv2.INTER_NEAREST #    INTER_NEAREST_EXACT     # cv2.INTER_LINEAR_EXACT # 
    ...
    mask = cv2.resize(1*mask.astype('uint8'), dsize= dsize, interpolation= interpol_method).astype('bool')
        
    return img, mask


# load line profiler, type this in the consolu comamand line (yes, with the %):  %load_ext line_profiler
path_to_results ='./results/'
redirect_std_to_file = False
if  redirect_std_to_file:    # sending output to file                    
    out_file_name = path_to_results+ 'StdOut.text'
    print('Output sent to ', out_file_name)    
    sys.stdout = open(out_file_name,  'w')
tic = time.time()
data_pack = get_images_from_wardrobe(user_name='malrawi', verbose=False)
